...
namespace FEI2
{
    public class clsEntityDocument_Line_TaxTotal2 : clsBaseEntidad2
    {
        ...
        public clsEntityDocument_Line_TaxTotal2 cs_pxObtenerUnoPorId(string id)
        {
        ...
        public clsEntityDocument_Line_TaxTotal2(clsBaseConfiguracion2 local)
        {
            conf = local;
        ...
        public List<List<string>> cs_pxObtenerTodoPorId(string id)
        {
            ...
            catch (Exception)
            {
               ...
               // clsBaseLog.cs_pxRegistarAdd("clsEntityDocument_Line_TaxTotal2 cs_pxObtenerTodoPorId" + ex.ToString());
                return null;
            }
        }

        public List<clsEntityDocument_Line2> cs_pxObtenerTodoPorCabeceraId(string id)
        {
            List<clsEntityDocument_Line2> comprobante_detalle;
            try
            {
                comprobante_detalle = new List<clsEntityDocument_Line2>();
                OdbcDataReader datos = null;
                string sql = "SELECT * FROM " + cs_cmTabla + " WHERE cs_Document_Line_Id=" + id.ToString().Trim() + " ;";
                clsBaseConexion2 cn = new clsBaseConexion2(conf);
                OdbcConnection cs_pxConexion_basedatos = new OdbcConnection(cn.cs_prConexioncadenabasedatos);
                ...
                clsEntityDocument_Line2 detalle;

                while (datos.Read())
                {

                    detalle = new clsEntityDocument_Line2(conf);/*
            ...
            {
               // clsBaseMensaje.cs_pxMsgEr("ERR6", ex.ToString());
               // clsBaseLog.cs_pxRegistarAdd("clsEntityDocument_Line_TaxTotal2 cs_pxObtenerTodoPorCabeceraId" + ex.ToString());
                return null;
            }
        }

       
        public List<clsEntityDocument_Line_TaxTotal2> cs_fxObtenerTodoPorCabeceraId(string id)
        {
            var resultado = new List<clsEntityDocument_Line_TaxTotal2>();
            try
            {
                OdbcDataReader datos = null;
                string sql = "SELECT * FROM " + cs_cmTabla + " WHERE cs_Document_Line_Id =" + id.ToString().Trim() + " ;";
                ...
                while (datos.Read())
                {
                    var item = new clsEntityDocument_Line_TaxTotal2(conf);
                    int count = 0;
            ...
            {
               // clsBaseLog.cs_pxRegistarAdd("clsEntityDocument_Line_TaxTotal2 cs_fxObtenerTodoPorCabeceraId" + ex.ToString());
            }
            return resultado;
        }
    }
