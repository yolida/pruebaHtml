...
namespace FEI2
{
    ...
    [ProgId("clsEntityDocument_TaxTotal2")]
    public class clsEntityDocument_TaxTotal2 : clsBaseEntidad2
    {
        ...
        public clsEntityDocument_TaxTotal2 cs_pxObtenerUnoPorId(string id)
        {
        ...
        public clsEntityDocument_TaxTotal2(clsBaseConfiguracion2 local)
        {
            conf = local;
        ...
        public List<List<string>> cs_pxObtenerTodoPorCabeceraId(string id)
        {
            ...
            catch (Exception)
            {
               ...
               // clsBaseLog.cs_pxRegistarAdd("clsEntityDocument_TaxTotal2 cs_pxObtenerTodoPorCabeceraId " + ex.ToString());
                return null;
            }
        }

        public List<clsEntityDocument_TaxTotal2> cs_fxObtenerTodoPorCabeceraId(string id)
        {
            var resultado = new List<clsEntityDocument_TaxTotal2>();
            try
            {
                OdbcDataReader datos = null;
                string sql = "SELECT * FROM " + cs_cmTabla + " WHERE cs_Document_Id =" + id.ToString().Trim() + " ;";
                ...
                while (datos.Read())
                {
                    var item = new clsEntityDocument_TaxTotal2(conf);
                    int count = 0;
            ...
            {
               // clsBaseLog.cs_pxRegistarAdd("clsEntityDocument_TaxTotal2 cs_fxObtenerTodoPorCabeceraId " + ex.ToString());
            }
            return resultado;
        }
    }
