...
namespace FEI1
{
    ...
    public class clsEntityVoidedDocuments_VoidedDocumentsLine1 : clsBaseEntidad1
    {
        ...
        public clsEntityVoidedDocuments_VoidedDocumentsLine1 cs_fxObtenerUnoPorId(string Id)
        {
        ...
        /// <returns></returns>
        public clsEntityVoidedDocuments_VoidedDocumentsLine1 cs_fxObtenerUnoPorDocumentoPrincipalYDocumentoRelacionado(string Id_documentoprincipal, string Id_documentorelacionado)
        {
            clsEntityVoidedDocuments_VoidedDocumentsLine1 registro = new clsEntityVoidedDocuments_VoidedDocumentsLine1(conf);
            OdbcDataReader datos = null;
            try
            {
                
                string sql = "SELECT * FROM " + cs_cmTabla + " WHERE cs_VoidedDocuments_Id ='" + Id_documentorelacionado + "' AND cp6='" + Id_documentoprincipal + "';";
            ...
            catch (Exception EX)
            {
               ...
               // clsBaseLog.cs_pxRegistarAdd("clsEntityVoidedDocuments_VoidedDocumentsLine1 cs_fxObtenerUnoPorDocumentoPrincipalYDocumentoRelacionado " + EX.ToString());
            }
            int count = 0;
            while (datos.Read())
            {
        ...
        internal List<clsEntityVoidedDocuments_VoidedDocumentsLine1> cs_fxObtenerTodoPorCabeceraId(string cs_pr_PK_VoidedDocuments_Id)
        {
            List<clsEntityVoidedDocuments_VoidedDocumentsLine1> comprobante_detalle;
            try
            {
                comprobante_detalle = new List<clsEntityVoidedDocuments_VoidedDocumentsLine1>();
                OdbcDataReader datos = null;
                ...
                clsEntityVoidedDocuments_VoidedDocumentsLine1 detalle;
                while (datos.Read())
                {
                    detalle = new clsEntityVoidedDocuments_VoidedDocumentsLine1(conf);
            ...
            {
               // clsBaseLog.cs_pxRegistarAdd("clsEntityVoidedDocuments_VoidedDocumentsLine1 cs_fxObtenerTodoPorCabeceraId" + ex.ToString());
                return null;
            }
        }

        public void cs_fxDescartarDocumento(string Id_documentoprincipal, string Id_documentorelacionado)
        {
            try
            {
                clsEntityVoidedDocuments_VoidedDocumentsLine1 registro = new clsEntityVoidedDocuments_VoidedDocumentsLine1(conf).cs_fxObtenerUnoPorDocumentoPrincipalYDocumentoRelacionado(Id_documentoprincipal, Id_documentorelacionado);
                clsEntityDocument1 documentoprincipal = new clsEntityDocument1(conf).cs_fxObtenerUnoPorId(Id_documentoprincipal);
                documentoprincipal.Cs_pr_ComunicacionBaja = "";
                documentoprincipal.cs_pxActualizar(false);
                registro.cs_pxElimnar(false);
            }
            catch (Exception) {
               // clsBaseMensaje.cs_pxMsg("Error comunicacion de baja","Se ha producido un error al descartar los documentos.");
               // clsBaseLog.cs_pxRegistarAdd("clsEntityVoidedDocuments_VoidedDocumentsLine1 cs_fxDescartarDocumento " + ex.ToString());
            }
        ...
        public clsEntityVoidedDocuments_VoidedDocumentsLine1(clsBaseConfiguracion1 conf1)
