...
namespace FEI1
{
    public class clsEntityDocument_DespatchDocumentReference1 : clsBaseEntidad1
    {
        ...
        public clsEntityDocument_DespatchDocumentReference1 cs_pxObtenerUnoPorId(string id)
        {
        ...
        public clsEntityDocument_DespatchDocumentReference1(clsBaseConfiguracion1 conf1)
        {
            conf = conf1;
        ...
        public List<List<string>> cs_pxObtenerTodoPorId(string id)
        {
            ...
            catch (Exception)
            {
               // clsBaseMensaje.cs_pxMsgEr("ERR6", ex.ToString());
                //clsBaseLog.cs_pxRegistarAdd("clsEntityDocument_DespatchDocumentReference1 cs_pxObtenerTodoPorId" + ex.ToString());
                return null;
            }
        }

        public List<clsEntityDocument_DespatchDocumentReference1> cs_pxObtenerTodoPorCabeceraId(string id)
        {
            List<clsEntityDocument_DespatchDocumentReference1> guia_remision;
            try
            {
                guia_remision = new List<clsEntityDocument_DespatchDocumentReference1>();
                OdbcDataReader datos = null;
                string sql = "SELECT * FROM " + cs_cmTabla + " WHERE cs_Document_Id='" + id.ToString().Trim() + "';";
                clsBaseConexion1 cn = new clsBaseConexion1(conf);
                OdbcConnection cs_pxConexion_basedatos = new OdbcConnection(cn.cs_prConexioncadenabasedatos);
                cs_pxConexion_basedatos.Open();
                datos = new OdbcCommand(sql, cs_pxConexion_basedatos).ExecuteReader();
                clsEntityDocument_DespatchDocumentReference1 entidad;
                while (datos.Read())
                {
                    entidad = new clsEntityDocument_DespatchDocumentReference1(conf);
            ...
            {
               // clsBaseMensaje.cs_pxMsgEr("ERR6", ex.ToString());
               // clsBaseLog.cs_pxRegistarAdd("clsEntityDocument_DespatchDocumentReference1 cs_pxObtenerTodoPorCabeceraId" + ex.ToString());
                return null;
            }
        }

        public List<clsEntityDocument_DespatchDocumentReference1> cs_fxObtenerTodoPorCabeceraId(string id)
        {
            var resultado = new List<clsEntityDocument_DespatchDocumentReference1>();
            try
            {
                OdbcDataReader datos = null;
                string sql = "SELECT * FROM " + cs_cmTabla + " WHERE cs_Document_Id ='" + id.ToString().Trim() + "';";
                ...
                while (datos.Read())
                {
                    var item = new clsEntityDocument_DespatchDocumentReference1(conf);
                    int count = 0;
            ...
            {
                //clsBaseLog.cs_pxRegistarAdd("clsEntityDocument_DespatchDocumentReference1 cs_fxObtenerTodoPorCabeceraId" + ex.ToString());
            }
            return resultado;
        }
    }
