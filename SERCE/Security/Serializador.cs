...
namespace Security
{
    public class Serializador : ISerializador
    {
        ...
        async Task<string> ISerializador.GenerarXml<T>(T objectToSerialize)     // objectToSerialize => Instancia de la Clase
        {
            var task = Task.Factory.StartNew(() =>
            {
                var serializer = new XmlSerializer(objectToSerialize.GetType()); // XmlSerializer propio de la librería
                string resultado;
                ...
                using (var memStr = new MemoryStream())
                {
                    ...
                    {
                        serializer.Serialize(stream, objectToSerialize); // Serialize propio de la librería
                    }
                    resultado = Convert.ToBase64String(memStr.ToArray()); // Convierte el string a Base64
                }
                return resultado; // Devuelve un StreamWriter en Base64
            });
            return await task;
        }
        ...
        public async Task<string> GenerateZip(string tramaXml, string nombreArchivo) {
            var task = Task.Factory.StartNew(() =>
            {
                var memOrigen   =   new MemoryStream(Convert.FromBase64String(tramaXml));
                var memDestino  =   new MemoryStream();
